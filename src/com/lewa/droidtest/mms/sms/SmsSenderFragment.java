...
public class SmsSenderFragment extends Fragment{
    ...
    private void sendSms(String destination, String content) {
        ...
        final ArrayList<PendingIntent> piSentIntent = Lists.newArrayList(PendingIntent.getBroadcast(mActivity, 0, sentIntent, PendingIntent.FLAG_UPDATE_CURRENT));
        final ArrayList<PendingIntent> piDeliveryIntent = Lists.newArrayList(PendingIntent.getBroadcast(mActivity, 0, deliveryIntent, PendingIntent.FLAG_UPDATE_CURRENT));

        ArrayList<String> smsDivs = manager.divideMessage(content);
        Utils.logE(this, "sending content #" + content + "# to #" + destination + "# with: " + manager);
